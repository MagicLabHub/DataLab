...
double run(int size, int stride, double FreqHZ)
{
	double szDataMB = (double)size;			// MB
	double assupTime = 0;
	double Throughout = 0.0;
    double cycles;
    int elemNum = size / sizeof(int); 
    test(elemNum, stride);                     /* warm up the cache */
    cycles = fcyc2(test, elemNum, stride, 0);  /* call test(elems,stride) */

	assupTime = cycles*1000000.0/FreqHZ;	 //	micro-second
	Throughout = szDataMB*FreqHZ/(1024*1024*stride*cycles);
    return Throughout; /* convert cycles to MB/s */
}
/* $end mountainfuns */
