...
struct PerfInfo
{
	...
	UINT64 curTemp;			// CPU Temperature
	UINT64 cSMICount;		// System Management Interrupts (SMIs)系统管理中断
	PerfInfo()
		: InstRetiredAny(0)
...
struct TopologyEntry // decribes a core
{
	...
	UINT32 thread_id;
	UINT32 core_id;
	UINT32 tile_id; // tile is a constalation of 1 or more cores sharing salem L2 cache. Unique for entire system
	UINT32 socket;

	TopologyEntry() : os_id(0), thread_id (0), core_id(0), tile_id(0), socket(0) { }
};
...
public:
	...
	UINT32 getCoreNum();
	std::vector<PerfInfo>  getPerfInfo();
...
private:
	...
	void getMemInfo();
	int getSystemInfo();
	PerfInfo readAndAggregate(UINT32 iCore);
...
private:
	std::vector<PerfInfo> m_PerfInfo;
