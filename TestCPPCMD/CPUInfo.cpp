...
UINT64 CPUInfo::readMSR(UINT64 msrNumber){
	...
	UINT64 Value = 0;
	DWORD eax=0,edx=0;
	ThreadGroupTempAffinity affinity(m_CoreIdx);
...
void CPUInfo::getCPUInfo(){
	...
	bool IsCPUSupport =( cpu_model == NEHALEM_EP
	...
	if(!Frequency){
		//UINT64 before = readMSR(IA32_TIME_STAMP_COUNTER);
		Frequency = get_frequency_from_cpuid();
	}
	...
	discoverSystemTopology();

	m_PerfInfo.reserve(num_cores);
	//for (UINT32 i = 0; i < num_cores; ++i){
	//	m_PerfInfo
	//	m_PerfInfo.push_back();
	//}
...
int CPUInfo::getSystemInfo()
{
	...
	*((int*)CPUString) = cpuid[1];
	*((int*)(CPUString+4)) = cpuid[3];
	*((int*)(CPUString+8)) = cpuid[2];
	...
	for (i=0; i<=nIds; ++i)
	{
		...
		if  (i == 1)
		{
			nSteppingID = cpuid[0] & 0xf;
			nModel = (cpuid[0] >> 4) & 0xf;
			nFamily = (cpuid[0] >> 8) & 0xf;
			nProcessorType = (cpuid[0] >> 12) & 0x3;
			nExtendedmodel = (cpuid[0] >> 16) & 0xf;
			nExtendedfamily = (cpuid[0] >> 20) & 0xff;
			nBrandIndex = cpuid[1] & 0xff;
			nCLFLUSHcachelinesize = ((cpuid[1] >> 8) & 0xff) * 8;
			nAPICPhysicalID = (cpuid[1] >> 24) & 0xff;
			bSSE3NewInstructions = (cpuid[2] & 0x1) || false;
			bMONITOR_MWAIT = (cpuid[2] & 0x8) || false;
			bCPLQualifiedDebugStore = (cpuid[2] & 0x10) || false;
			bThermalMonitor2 = (cpuid[2] & 0x100) || false;
			nFeatureInfo = cpuid[3];
		}
	}
	...
	for (i=0x80000000; i<=nExIds; ++i)
	{
		...
		if  (i == 0x80000002)
			memcpy(CPUBrandString, cpuid, sizeof(cpuid));
		else if  (i == 0x80000003)
			memcpy(CPUBrandString + 16, cpuid, sizeof(cpuid));
		else if  (i == 0x80000004)
			memcpy(CPUBrandString + 32, cpuid, sizeof(cpuid));
		else if  (i == 0x80000006)
		{
			nCacheLineSize = cpuid[2] & 0xff;
			nL2Associativity = (cpuid[2] >> 12) & 0xf;
			nCacheSizeK = (cpuid[2] >> 16) & 0xffff;
		}
	}
...
PerfInfo CPUInfo::readAndAggregate(UINT32 iCore)
{
	//SetThreadAffinityMask(GetCurrentThread(),iCore);
	m_CoreIdx = iCore;
	...
	m_PerfInfo[m_CoreIdx].InstRetiredAny		+= cInstRetiredAny;
	m_PerfInfo[m_CoreIdx].CpuClkUnhaltedThread += cCpuClkUnhaltedThread;
	m_PerfInfo[m_CoreIdx].CpuClkUnhaltedRef	+= cCpuClkUnhaltedRef;
	m_PerfInfo[m_CoreIdx].L3Miss				+= cL3Miss;
	m_PerfInfo[m_CoreIdx].L3UnsharedHit		+= cL3UnsharedHit;
	m_PerfInfo[m_CoreIdx].L2HitM				+= cL2HitM;
	m_PerfInfo[m_CoreIdx].L2Hit				+= cL2Hit;
	m_PerfInfo[m_CoreIdx].InvariantTSC			+= cInvariantTSC;
	m_PerfInfo[m_CoreIdx].curTemp				= (readMSR(MSR_IA32_THERM_STATUS)& 0x7f0000) >> 16;
	m_PerfInfo[m_CoreIdx].cSMICount			= readMSR(MSR_SMI_COUNT);//System Management Interrupts (SMIs)系统管理中断
	return m_PerfInfo[m_CoreIdx];
}
...
std::vector<PerfInfo> CPUInfo::getPerfInfo(){
	CPUInfo * inst = CPUInfo::getInstance();
	std::vector<PerfInfo> result;
	for(int i =0;i<num_cores;++i){
		PerfInfo coreInfo = inst->readAndAggregate(i);
		result.push_back(coreInfo);
	}
	return result;
}

UINT32 CPUInfo::getCoreNum(){
	return num_cores;
}

UINT64 CPUInfo::get_frequency_from_cpuid()
{
	char buffer[sizeof(int)*4*3+1];
	memset(buffer, 0, sizeof(int)*4*3+1);
	int* info = (int*) buffer;
	int cpuid[4];
	__cpuid(cpuid, 0x80000002); 
	memcpy(info,cpuid,sizeof(int)*4);
	info += 4;
	...
	std::string strCPUBrand(buffer);
	while(strCPUBrand[0]==' ') strCPUBrand.erase(0,1);
	std::string::size_type i;
	while((i = strCPUBrand.find("  ")) != std::string::npos) strCPUBrand.replace(i,2," "); // remove duplicate spaces

	double speed=0;
	if (strCPUBrand.length() > std::string::size_type(0))
	{
		std::string::size_type unitsg = strCPUBrand.find("GHz");
		if(unitsg != std::string::npos)
		{
			std::string::size_type atsign = strCPUBrand.rfind(' ', unitsg);
			if(atsign != std::string::npos)
			{
				std::istringstream(strCPUBrand.substr(atsign)) >> speed;
				speed *= 1000;
			}
		}
		else
		{
			std::string::size_type unitsg = strCPUBrand.find("MHz");
			if(unitsg != std::string::npos)
			{
				std::string::size_type atsign = strCPUBrand.rfind(' ', unitsg);
				if(atsign != std::string::npos)
...
bool CPUInfo::discoverSystemTopology(){
...

	{
		...
		do
		{
			__cpuidex(cpuinfo,0xb, subleaf);
			if (cpuinfo[1] == 0)
			{ // if EBX ==0 then this subleaf is not valid, we can exit the loop
				break;
			}
			levelType = extract_bits_ui(cpuinfo[2], 8, 15);
			levelShift = extract_bits_ui(cpuinfo[0], 0, 4);
			switch (levelType)
			{
	...
	__cpuidex(cpuinfo,0x4, 2);
	l2CacheMaskWidth = 1 + extract_bits_ui(cpuinfo[0],14,25); // number of APIC IDs sharing L2 cache
	...
	for (int i = 0; i < (int)num_cores; i++)
	{
		...
		__cpuidex(cpuinfo,0xb, 0x0);
		int apic_id = cpuinfo[3];

		TopologyEntry entry;
		entry.os_id = i;
		entry.thread_id = extract_bits_ui((UINT32)apic_id, 0, smtMaskWidth - 1);
